...
class CoreRedactorEngine:
    ...
    def redact_all(self, line: str) -> tuple:
        ...
        for id in id_object.regexes:
            redact_pattern = id['pattern']
            if re.search(redact_pattern, line) and "[REDACTED]" not in line:
                pattern_string = re.search(redact_pattern, line).group(0)
                line = re.sub(redact_pattern, "[REDACTED]", line)
                hash_map.update({pattern_string: "[REDACTED]"})
